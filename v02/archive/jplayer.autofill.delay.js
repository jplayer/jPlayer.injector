// This version creates a custom event called "loadVideo" that you can trigger on the
// jQuery selctor: $(".jp-jplayer").trigger("loadVideo");
// This delays instantiation until some later time, like the element becoming visible
// Useful when you have a large amount of videos all living in the DOM at the same time
var autofill = function(selector,template){
  ...
  $.get(template,function(video_template){
    $(selector).each(function(key,val){
      ...
      //// VIDEO
      $("#jquery_jplayer_"+key).bind("loadVideo",function(){
        $(this).jPlayer({
          ready: function () {
